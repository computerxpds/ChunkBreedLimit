...
public class BreederListener implements Listener {
	...
	public BreederListener(ChunkBreedLimit plugin, int cap,
			List<EntityType> allowedEnts, String bmsg) {
	...
	public void onCreatureSpawnEvent(CreatureSpawnEvent event) {
		if (event.getSpawnReason() == CreatureSpawnEvent.SpawnReason.BREEDING
			...
			if (individualEntityCap
					&& allowedEntities.contains(event.getEntityType())) {
				event.setCancelled(checkAndApplyEntityWithCap(
						event.getEntityType(), event.getLocation().getChunk(),
	...
	public boolean checkAndApplyEntityWithCap(EntityType ent, Chunk chunk,
			Integer cap) {
		/*
