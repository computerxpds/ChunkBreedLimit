...
public class ChunkBreedLimit extends JavaPlugin {
	...
	public void onEnable() {
		...
		boolean individualCap = this.getConfig().getBoolean(
				"use-individual-cap", false);
		String breedingFailMessage = this.getConfig().getString(
				"breeding-fail-msg",
				"The animal cannot breed - it is too crowded!");

		if (individualCap) {
			List<String> entityListLoad = this.getConfig().getStringList(
					"entity-list-individual");
			if (entityListLoad.size() == 0) {
				getLogger()
						.info("Entity list is invalid for individual cap loading! Reverting to defaults..");
				...
				for (String entry : entityListLoad) {
					...
					EntityType temptype = EntityType.fromName(temp[0]
							.toUpperCase());
					getLogger()
							.info(String.format("%s, %s", temptype, tempint));
					individCapData.put(temptype, tempint);
				}
			}

			new BreederListener(this, entitySpawnCap, individCapData,
					breedingFailMessage);
			...
			if (entityListLoad.size() == 0) {
				getLogger().info(
						"Entity list appears invalid! Reverting to defaults..");
			...
			getLogger()
					.info(String
							.format("Now watching: %s when entity chunk count above %s",
									allowedEntityList.toString(),
									entitySpawnCap));
		}
